...
% UTORid>.
% 
% MAT292 Fall 2019, Stinchcombe & Parsch MAT292 Fall 2018, Stinchcombe & Khovanskii 
% MAT292 Fall 2017, Stinchcombe & Sinnamon MAT292 Fall 2016, Sinnamon & Sousa 
...
% Initialize vectors and compute delta
x = zeros(101,1);
y = zeros(101,1);
delta = (7-(-3))/(101-1);

% Compute y_i value for each x_i value
for i = 1:101
    x(i) = -3 + (i-1)*delta;
    y(i) = abs(x(i))*x(i);
end
...
% Get computation time for y_i values using for loop
clear y;
tic;
for i = 1:5000001
    y(i) = atan(x(i));
...
% Get computation time for y_i values using vectorization
clear y;
tic;
y = atan(x);
toc;
...
% Initialize t vector and a vector for each specific solution
t = linspace(1,5);
...
xlabel('t');
ylabel('y(t)');
title('Solution plot of ODE with three different inital conditions');
...
% Print computed solutions at specified x values
fprintf(' y(%g) = %g\n', 0, fzero(@(y) f(0, y), 0.7));
fprintf(' x(%g) = %g\n', 1/2, fzero(@(x) f(x, 1/2), 0.6));
fprintf(' x(%g) = %g\n', 1/2, fzero(@(x) f(x, 1/2), 1.1));
...
% 100 points equally spaced from -2 to 2, inclusi
xvals = linspace(-2, 2); 
...
% general solution is f(x,y) = abs(y) - e^(-sinx + x^3 +c) = 0
% particular solution is f(x,y) = abs(y) - e^(-sinx + x^3) = 0

% Specify inline function
f = @(x,y) abs(y) - exp((-sin(x) + x.^3));
...
for j = 1:length(xvals)
   ...
   yvals(j) = fzero(@(y) f(xvals(j), y), 0.5);
...
% 
% Iode has a problem (with MATLAB versions 2014a and above), but can be easily 
% fixed. In MATLAB open the folder where you installed iode and double-click the 
% file fixaxes.m. Quote (comment out) line 38: it should look like %eval(’delete(a:);’); 
% Do the same for the file freeaxes.m and line 24.
%% Exercise 11
...
%If y(x) = 0, y remains 0 for all x.
%If y(x) > 0 for all x, y goes to positive infinity as x
%goes to positive and negative infinity.
%If y(x) < 0 for all x, y asymptotes to 0 from below as x
%goes to positive and negative infinity.
%
%The solutions are symmetric because y(x) is an even function.
%Since dy/dx is a seperable equation, it can easily be shown that
%the solution is y(x) = -1/((1/2)*x^2 + C), which is an even function.
%% Exercise 12
...
%As x approaches infinity, the solutions approach
%either an equilibrium point or positive infinity.
%
%If y_0 < -1, the solutions approach -1 as x goes to positive infinity.
%If -1 < y_0 < 1, the solutions approach 1 from the bottom as x goes to positive infinity.
%If 1 < y_0 < 2, the solutions approach 1 from the top as x goes to positive infinity.
%If y_0 > 2, the solutions diverge to positive infinity as x goes to positive infinity.
%If the solution is a constant function at one of the
%equilibrium points, the solution remains at the equilibrium
%point as x goes to positive infinity.
%
%When using the Euler solution method, solutions crossed and thus
%the Runge-Kutta method was used instead.
