...
% modified from MAT292, Fall 2015, Sousa, based on Spring 2010, Jerrard
%% Student Information
%%
% 
...
% 
% Change the window to |[0,10]x[-3,3]|. Save a cropped screenshot with the filename 
% ex1_<UTORid>.png Changing "UTORid" below will result in the image being included 
% when you "Publish".
% 
% <<\\SRVD\Homes$\chaud496\Documents\MAT 292\Lab 5\ex1_chaud496.png>>
% 
...
% which continuously oscillate for t in [0,inf). the multiplication by a
% decaying exponential (exponential decays for t in [0,inf) due to
% negative powers) results in decaying oscillatory motion for t in [0,inf).
%% Exercise 2
% Consider the ODE:
%%
% 
...
% 
% <<\\SRVD\Homes$\chaud496\Documents\MAT 292\Lab 5\ex2_chaud496.png>>
...
% Repeat (a), (b), (c) from Exercise 1 with this DE.
% 
% <<\\SRVD\Homes$\chaud496\Documents\MAT 292\Lab 5\ex3_chaud496.png>>
...
% exact solution:
% y(t) = c_1*e^(-t)*sin(2*t) + c_2*sin(t) + c_3*e^(-t)*cos(2*t) + c_4*cos(t)

% for most initial conditions, the solution will oscillate without growing
% or decaying. the e^(-x)*sin(2*x) and e^(-x)*sin(2*x) terms go to zero as
% t goes to positive infinity (this is due to the exponentials with negative powers).
% the remaining sine and cosine terms will continuously oscillate. in the specific
...
%%
% SUMMARY:
% for real roots, solutions decay or grow based on the sign and magnitude
% of r1 and r2 (root > 0 is growth, root < 0 is decay). A solution with one
% or two positive roots will grow whereas a solution with two negative
% roots will decay.
%
% for complex roots, solutions will oscillate while growing, decaying or
% neither. the real component determines the rate of growth or decay (the
% lack of a real component means the solution will oscillate without growth
% or decay). the complex component determines the rate of oscillation.
...
% 
% <<\\SRVD\Homes$\chaud496\Documents\MAT 292\Lab 5\ex7_chaud496.png>>
...
% approximation. The IODE solution diverges away from the second-order ODE
% solution whenever the function approaches a local maximum or minimum.
% Otherwise, its approximation closely matches the approximation made by my
% function.